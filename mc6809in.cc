...
//	(C) R.P.Bellis
//
//	Updated and corrected by BDA and
//	Soren Roug - December 2003
//
...
{
	// BDA - ANDB bug fix (looks like a typo)
//	help_add(b);
	help_and(b);
}
...
{
//	printf("bef %d %d %02x %04x\n",cc.bit.i,cc.bit.f,cc.all,pc);
	cc.all &= fetch();
...
{
	// BDA - This is wrong according to
	// Color Computer Assembly Language Programming
	// P. 74 - Changing BGT and LBGT accordingly
...
{
	// BDA - carry fix, with thanks to Soren Roug
	// Complete rewrite of function
	Byte    m = fetch_operand();
	int t = x - m;
...
{
	Word	addr = fetch_effective_address();
	// BDA - Remove warning msg from compiler
//	write(--s, pc);
	write(--s, (pc & 0xff));
	write(--s, pc >> 8);
	pc = addr;
...
{
	// BDA - Determined rewrite of
	// function, after some input by Soren Roug
	int t = 0 - x;
...
void mc6809::help_pul(Byte w, Word& s, Word& u)
{
	...
	if (btst(w, 4)) {
		x = ((Word)read(s++))<<8; x |= read(s++);
	}
	if (btst(w, 5)) {
		y = ((Word)read(s++))<<8; y |= read(s++);
	}
	if (btst(w, 6)) {
		u = ((Word)read(s++))<<8; u |= read(s++);
	}
	if (btst(w, 7)) {
		pc = ((Word)read(s++))<<8; pc |= read(s++);
	}
//	printf("help_pul %02x %04x %04x\n", w, s, pc);
}
