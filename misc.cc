...
char *hexstr(Word x)
{
	...
	// BDA - force casting of parameters to avoid warnings
	tmp[3] = hex_digit((unsigned char)x);	x >>= 4;
	tmp[2] = hex_digit((unsigned char)x);	x >>= 4;
	tmp[1] = hex_digit((unsigned char)x);	x >>= 4;
	tmp[0] = hex_digit((unsigned char)x);
