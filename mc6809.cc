...
{
	// BDA - Increase memory to 1 Meg to support
	// telescope controller bank switching.
	// For most uses this can be set back to 10000L
	memory = new Byte[0x10000L];
	// BDA - I comment out the following line, since your
	// derived function should call reset() in its
...
//		case 0x00: case 0x60: case 0x70:
		...
		case 0x1029:
			lbvs(); break;
		case 0x113d:
			mode=immediate;fetch_operand();break;
		default:
	...
	if ((post & 0x80) == 0x00) {
		...
		switch (post & 0x1f) {
			...
			case 0x04: case 0x14:
				...
				break;
			case 0x05: case 0x15:
				// BDA - error!  Need to sign extend register first!
//				addr = b + refreg(post);
...
//				addr = a + refreg(post);
			...
			case 0x0b: case 0x1b:
				...
				break;
			case 0x0c: case 0x1c:
				// BDA - fix needed to handle PCR addressing
				// Have to update PC first before the add!
